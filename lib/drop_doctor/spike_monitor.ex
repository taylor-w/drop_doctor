defmodule DropDoctor.SpikeMonitor do
  @moduledoc """
  Continuous, high-rate stability sampler for one host.

  The 5-second sweep (`DropDoctor.Monitor`) answers "whose fault is the outage?"
  ...
  latency / brief loss via `DropDoctor.Stability`.
  ...
  wire rather than the scheduler. See `DropDoctor.Probes.Ping.stream/3`.
  ...
  alias DropDoctor.{Aggregate, Measurements, Probes.Ping, Probes.TcpStream, Stability}
  ...
  def handle_cast(:reset, state) do
    stats = Stability.empty()
    Phoenix.PubSub.broadcast(DropDoctor.PubSub, @topic, {:stability, state.key, stats})
  ...
  def handle_info({:hosts_selected, reachable}, state) do
    ...
    if primary != state.host and state.running do
      ...
      Phoenix.PubSub.broadcast(DropDoctor.PubSub, @topic, {:stability, state.key, state.stats})
  ...
  defp log_events(
    ...
    spawned =
      Task.Supervisor.start_child(DropDoctor.ProbeSupervisor, fn ->
        try do
