defmodule TrackConn.Monitor do
  @moduledoc """
  ...
  Design goals addressed here:
    ...
    * **Trustworthy verdicts (Tier 1).** Keeps a rolling window of recent raw
      sweeps and diagnoses from their *median* (see `TrackConn.Aggregate`), so a
  ...
  alias TrackConn.{Aggregate, Diagnosis, Measurements, Sweeper, Targets}
  ...
  def subscribe(server \\ __MODULE__),
    do: Phoenix.PubSub.subscribe(TrackConn.PubSub, topic(server))
  ...
  defp start_sweep(%{running: true, task: nil} = state) do
    ...
    task =
      Task.Supervisor.async_nolink(TrackConn.ProbeSupervisor, fn -> Sweeper.run(ladder, opts) end)
  ...
  defp process_sweep(state, raw_sweep) do
    ...
    Phoenix.PubSub.broadcast(TrackConn.PubSub, state.topic, {:sweep, verdict, row})
