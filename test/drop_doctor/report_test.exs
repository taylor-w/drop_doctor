defmodule DropDoctor.ReportTest do
  ...
  defp spike_events do
    [
      %DropDoctor.Measurements.SpikeEvent{
  ...
  describe "filename/2" do
    test "embeds the timestamp and extension" do
      assert Report.filename(:csv, @now) == "drop_doctor-isp-report-2026-06-05_1300Z.csv"
      assert Report.filename(:html, @now) == "drop_doctor-isp-report-2026-06-05_1300Z.html"
      assert Report.filename(:spikes, @now) == "drop_doctor-spike-log-2026-06-05_1300Z.csv"
    end
  end
  ...
  describe "spikes_csv/1 and the stability section" do
    ...
    test "CSV neutralizes spreadsheet formula-injection prefixes in text cells" do
      evil = %DropDoctor.Measurements.SpikeEvent{
        occurred_at: ~U[2026-06-05 12:59:40Z],
        segment: "internet",
        host: "=2+2",
        kind: "latency",
