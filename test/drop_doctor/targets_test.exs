defmodule DropDoctor.TargetsTest do
  ...
  setup do
    ...
    saved_cfg = Application.get_env(:drop_doctor, :targets)
    ...
    on_exit(fn ->
      ...
      if saved_cfg do
        Application.put_env(:drop_doctor, :targets, saved_cfg)
      else
        Application.delete_env(:drop_doctor, :targets)
      end
    end)
  ...
  describe "internet_anchors/0 robustness" do
    test "an empty :internet_anchors config falls back to defaults (no hd([]) crash)" do
      Application.put_env(:drop_doctor, :targets, %{internet_anchors: []})
    ...
    test "a blank INTERNET_IP env is treated as unset, not a [\"\"] anchor" do
      put_env("INTERNET_IP", "")
      Application.put_env(:drop_doctor, :targets, %{internet_anchors: ["9.9.9.9"]})
  ...
  describe "router_target/0 robustness" do
    test "a blank ROUTER_IP env is ignored rather than probed as an empty host" do
      put_env("ROUTER_IP", "")
      Application.put_env(:drop_doctor, :targets, %{router: "10.0.0.1"})
  ...
  describe "target validation (no CLI-flag injection into the probes)" do
    ...
    test "a flag-like INTERNET_IP is rejected and falls back to defaults" do
      ...
      Application.put_env(:drop_doctor, :targets, %{})
      anchors = Targets.internet_anchors()
    ...
    test "an injected config anchor is dropped, valid ones kept" do
      Application.put_env(:drop_doctor, :targets, %{internet_anchors: ["-x", "9.9.9.9"]})
      assert Targets.internet_anchors() == ["9.9.9.9"]
    end

    test "hostnames and IPv6 are accepted" do
      Application.put_env(:drop_doctor, :targets, %{dns: "cloudflare.com"})
      assert Targets.dns_target() == "cloudflare.com"

      Application.put_env(:drop_doctor, :targets, %{internet: "2606:4700:4700::1111"})
